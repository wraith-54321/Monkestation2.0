...
/datum/martial_art/jungle_arts/proc/jungle_attack(mob/living/attacker, mob/living/defender, grab_attack)
	...
	switch(rand(1,6))
		if(1)
			atk_verb = "dragged"
			var/obj/item/organ/external/tail/tail = attacker.get_organ_slot(ORGAN_SLOT_EXTERNAL_TAIL)
			...
			playsound(attacker, 'sound/weapons/whip.ogg', 50, TRUE, -1)
		...
		if(6)
			...
			playsound(attacker, 'sound/weapons/bite.ogg', 50, TRUE, -1)
