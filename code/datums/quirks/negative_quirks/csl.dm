...
/datum/quirk/csl/proc/get_native_language()
	...
	var/obj/item/organ/internal/tongue/tongue = quirk_holder.get_organ_by_type(/obj/item/organ/internal/tongue)
...
/datum/quirk/csl/proc/translate_parts(datum/source, list/say_args)
	...
	LAZYINITLIST(say_args[LANGUAGE_MUTUAL_BONUS])
	// force speak language, add mutual bonuses so everyone else can understand
	say_args[SPEECH_LANGUAGE] = native_language
	say_args[LANGUAGE_MUTUAL_BONUS][native_language] = max(round(8 * sqrt(quirk_holder.mob_mood?.sanity), 5), say_args[LANGUAGE_MUTUAL_BONUS][native_language])
