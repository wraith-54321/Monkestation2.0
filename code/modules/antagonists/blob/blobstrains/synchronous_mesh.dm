...
/datum/blobstrain/reagent/synchronous_mesh/damage_reaction(obj/structure/blob/damaged, damage, damage_type, damage_flag)
	if(damage_flag == MELEE || damage_flag == BULLET || damage_flag == LASER) //the cause isn't fire or bombs, so split the damage
		...
		var/list/damaged_list = list()
		for(var/obj/structure/blob/new_damaged in orange(1, damaged))
			if(!istype(new_damaged, /obj/structure/blob/special) && new_damaged.blob_team.blobstrain.type == src.type)
				damagesplit += 1
				damaged_list += new_damaged

		for(var/obj/structure/blob/damaged_tile in damaged_list)
			damaged_tile.take_damage(damage / damagesplit, CLONE, FALSE, FALSE)
