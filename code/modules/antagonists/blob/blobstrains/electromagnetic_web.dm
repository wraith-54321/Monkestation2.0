...
/datum/blobstrain/reagent/electromagnetic_web/damage_reaction(obj/structure/blob/damaged, damage, damage_type, damage_flag)
	if(damage_type == BRUTE) // take full brute, divide by the multiplier to get full value
		return damage / damaged.brute_resist
	return damage * 1.25 //a laser will do 25 damage, which will kill any normal blob

/datum/blobstrain/reagent/electromagnetic_web/death_reaction(obj/structure/blob/dying, damage_flag)
	if(damage_flag == MELEE || damage_flag == BULLET || damage_flag == LASER)
		empulse(get_turf(dying), 1, 3) //less than screen range, so you can stand out of range to avoid it
