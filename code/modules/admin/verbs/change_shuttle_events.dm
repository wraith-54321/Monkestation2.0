ADMIN_VERB(change_shuttle_events, R_ADMIN | R_FUN, FALSE, "Change Shuttle Events", "Change the events on a shuttle.", ADMIN_CATEGORY_EVENTS)
	...
	if(result == "Clear")
		port.event_list.Cut()
		log_admin("[key_name_admin(user)] has cleared the shuttle events on: [port]")
	else if(options[result])
		var/typepath = options[result]
		if(typepath in active)
			port.event_list.Remove(active[options[result]])
			log_admin("[key_name_admin(user)] has removed '[active[result]]' from [port].")
		else
			port.event_list.Add(new typepath (port))
			log_admin("[key_name_admin(user)] has added '[typepath]' to [port].")
