...
/proc/force_event(event_typepath, cause)
	var/datum/round_event_control/our_event = SSevents.control_by_type[event_typepath]
	if(!our_event)
...
/proc/force_event_after(event_typepath, cause, duration)
	var/datum/round_event_control/our_event = SSevents.control[event_typepath]
	if(!our_event)
		CRASH("Attempted to force event [event_typepath], but the event path could not be found!")
