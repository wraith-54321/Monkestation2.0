//This element will add text_to_add to the examine() of an atom if check_requirements() passes
...
/datum/element/extra_examine/proc/add_examine(atom/source, mob/user, list/examine_texts)
	...
	if(check_requirementssource, user, examine_texts))
		examine_texts += text_to_add

/datum/element/extra_examine/proc/check_requirements(atom/source, mob/user, list/examine_texts)
	return TRUE
