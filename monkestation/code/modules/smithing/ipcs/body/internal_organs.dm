...
/obj/item/organ/internal/brain/synth/on_mob_insert(mob/living/carbon/organ_owner, special, movement_flags)
	. = ..()

	if(organ_owner.stat != DEAD || !ishuman(organ_owner))
		return

	var/mob/living/carbon/human/user_human = organ_owner
