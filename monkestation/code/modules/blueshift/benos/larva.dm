...
/datum/action/cooldown/alien/neutered_larva_evolve/Activate(atom/target)
	...
	var/alien_caste = show_radial_menu(owner, owner, caste_options1, radius = 38, require_near = TRUE, tooltips = TRUE)
	if(QDELETED(src) || QDELETED(owner) || !IsAvailable(feedback = TRUE) || isnull(alien_caste))
		return
...
/datum/action/cooldown/alien/neutered_larva_evolve/proc/make_neutered_xeno_caste_entry(caste_name, caste_image, caste_info, list/caste_options1)
	...
	caste_options1[caste_name] = caste_option

/datum/action/cooldown/alien/neutered_larva_evolve/proc/spawn_new_xeno(alien_caste)
	var/mob/living/carbon/alien/adult/nova/new_xeno
	var/mob/living/carbon/alien/larva/larva = owner
